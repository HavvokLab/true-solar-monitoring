...
type PlantAggregateService interface {
	UpdatePlantByDateToSQLite(date *time.Time) error
	UpdatePlantByDateToElastic(date *time.Time) error
}
...
func (s *plantAggregateService) UpdatePlantByDateToSQLite(date *time.Time) error {
...
func (s *plantAggregateService) UpdatePlantByDateToElastic(date *time.Time) error {
	index := fmt.Sprintf("%s-%s", config.GetConfig().Elastic.SolarIndex, date.Format("2006.01.02"))
	data, err := s.solarRepo.GetUniquePlantByIndex(index)
	if err != nil {
		s.logger.Error(err)
	...
	if err := s.solarRepo.DeleteIndex("master-site"); err != nil {
		s.logger.Error(err)
	}

	if err := s.solarRepo.BulkIndex("master-site", plants); err != nil {
		s.logger.Error(err)
		return err
	}
