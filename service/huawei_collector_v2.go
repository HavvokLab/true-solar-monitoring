...
type HuaweiCollectorV2Service interface {
	Run(*model.HuaweiCredential) error
}

type huaweiCollectorV2Service struct {
	vendorType     string
...
func (s *huaweiCollectorV2Service) Run(credential *model.HuaweiCredential) error {
	defer func() {
		if r := recover(); r != nil {
...
func (s *huaweiCollectorV2Service) run(credential *model.HuaweiCredential, documentCh chan interface{}, doneCh chan bool, errorCh chan error) {
	now := time.Now()
	...
	s.logger.Infof("[%v] - HuaweiCollectorService.run(): %v plants found", credential.Username, len(plantList))
	...
	for _, station := range plantList {
		...
		if station.PlantCode != nil {
			stationCodeList = append(stationCodeList, station.GetPlantCode())
		}
	}
	stationCodeListString = append(stationCodeListString, strings.Join(stationCodeList, ","))
	...
	for _, station := range plantList {
		stationCode := station.GetPlantCode()
