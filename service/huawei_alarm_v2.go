...
type HuaweiAlarmV2Service interface {
	Run(*model.HuaweiCredential) error
}

type huaweiAlarmV2Service struct {
	snmpRepo    repo.SnmpRepo
	redisClient *redis.Client
	logger      logger.Logger
}

func NewHuaweiAlarmV2Service(
	snmpRepo repo.SnmpRepo,
	redisClient *redis.Client,
	logger logger.Logger,
) HuaweiAlarmService {
	return &huaweiAlarmV2Service{
		snmpRepo:    snmpRepo,
...
func (s *huaweiAlarmV2Service) Run(credential *model.HuaweiCredential) error {
	now := time.Now()
	...
	s.logger.Infof("[%v] - huaweiAlarmService.Run(): plant size: %v", credential.Username, len(plantList))
	...
	for _, plant := range plantList {
		...
		if plant.PlantCode != nil {
			stationCodeList = append(stationCodeList, plant.GetPlantCode())
		}
	}
	stationCodeListString = append(stationCodeListString, strings.Join(stationCodeList, ","))
